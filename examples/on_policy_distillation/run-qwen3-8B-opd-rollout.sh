...
# usage: bash examples/on_policy_distillation/run-qwen3-8B-opd-rollout.sh
#
# Rollout-only OPD debugging: launch the student rollout engines and the
# external teacher server without loading a Megatron training model.
...
CKPT_ARGS=(
   ...
   --load /opt/tiger/models/Qwen3-8B_slime_teacher_offload/
   --save /opt/tiger/models/Qwen3-8B_slime_teacher_offload/
   --save-interval 50
...
# Teacher occupies GPUs 6,7 (TP=2), so all remaining GPUs 0-5 are assigned to
# six single-GPU rollout engines. --debug-rollout-only prevents Megatron model,
# optimizer, training, weight-update, and checkpoint initialization.
